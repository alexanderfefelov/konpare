...
import com.github.alexanderfefelov.konpare.syntax.{Subject, Syntax}

object Snmp extends Subject {

  override def process2(data: List[String], model: collection.mutable.Map[String, String]) = {
    data.head match {
      case Syntax.PARAMETER_SYSTEM_NAME =>
        // config snmp system_name FOOBAR
        Parser.addPairsToModel(data, model, s"${Syntax.SUBJECT_SNMP}")
      case _ =>
    }
  }
